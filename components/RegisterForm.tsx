"use client";

/**
 * RegisterForm Component
 *
 * Note: Hydration mismatch warnings on this form can be caused by browser extensions
 * (e.g., LastPass, password managers) that inject extra DOM nodes (like <div data-lastpass-icon-root>)
 * into form elements on the client side only. These injections occur after initial render
 * and cause React to detect a mismatch between server and client HTML.
 *
 * This is not a code issue - the component JSX is fully deterministic. To avoid these warnings
 * during development, disable password manager extensions or use an incognito/private window
 * without extensions.
 */

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { auth } from "@/lib/firebase";
import { createUserWithEmailAndPassword, sendEmailVerification, User } from "firebase/auth";
import { PrimaryButton } from "./PrimaryButton";
import { Card } from "./Card";

function getAuthErrorMessage(error: any): string {
  const code = error?.code || "";
  switch (code) {
    case "auth/email-already-in-use":
      return "Questa email è già registrata. Prova ad accedere invece.";
    case "auth/invalid-email":
      return "L'indirizzo email non è valido.";
    case "auth/operation-not-allowed":
      return "Questa operazione non è consentita. Contatta il supporto.";
    case "auth/weak-password":
      return "La password è troppo debole. Usa almeno 6 caratteri.";
    case "auth/network-request-failed":
      return "Errore di connessione. Controlla la tua connessione internet.";
    case "auth/too-many-requests":
      return "Troppi tentativi. Riprova più tardi.";
    default:
      return error?.message || "Errore durante la registrazione";
  }
}

export function RegisterForm() {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [info, setInfo] = useState<string | null>(null);
  const [debugUser, setDebugUser] = useState<User | null>(null);

  useEffect(() => {
    if (process.env.NODE_ENV === "development") {
      const unsubscribe = auth.onAuthStateChanged((user) => {
        setDebugUser(user);
      });
      return () => unsubscribe();
    }
  }, []);

  const handleRegister = async (email: string, password: string) => {
    try {
      // 1. Create the user
      const cred = await createUserWithEmailAndPassword(auth, email, password);

      // 2. Send the verification email
      await sendEmailVerification(cred.user, {
        url: window.location.origin + "/verify-email",
        handleCodeInApp: true,
      });

      // 3. Return a clear result to the UI
      return {
        success: true,
        message: "Email di verifica inviata. Controlla la tua casella di posta.",
      };
    } catch (error: any) {
      console.error("Error during register / email verification:", error);
      return {
        success: false,
        message:
          getAuthErrorMessage(error) ||
          error?.message ||
          "Registrazione o invio email di verifica fallito.",
      };
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    setInfo(null);

    if (confirmPassword && password !== confirmPassword) {
      setError("Le password non corrispondono");
      setLoading(false);
      return;
    }

    const result = await handleRegister(email, password);

    if (result.success) {
      setInfo(result.message);
      // Optionally redirect after showing the message for a moment
      setTimeout(() => {
        router.push("/dashboard");
      }, 3000);
    } else {
      setError(result.message);
    }

    setLoading(false);
  };

  return (
    <Card className="p-8">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label
            htmlFor="email"
            className="block text-sm font-medium text-gray-300 mb-2"
          >
            Email
          </label>
          <input
            id="email"
            type="email"
            className="w-full px-4 py-3 bg-zinc-800/50 backdrop-blur-sm border border-zinc-700/50 rounded-xl focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500/50 transition-all duration-200 text-white placeholder:text-gray-500"
            placeholder="nome@esempio.com"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            required
            disabled={loading}
          />
        </div>

        <div>
          <label
            htmlFor="password"
            className="block text-sm font-medium text-gray-300 mb-2"
          >
            Password
          </label>
          <input
            id="password"
            type="password"
            className="w-full px-4 py-3 bg-zinc-800/50 backdrop-blur-sm border border-zinc-700/50 rounded-xl focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500/50 transition-all duration-200 text-white placeholder:text-gray-500"
            placeholder="••••••••"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            required
            disabled={loading}
          />
        </div>

        <div>
          <label
            htmlFor="confirmPassword"
            className="block text-sm font-medium text-gray-300 mb-2"
          >
            Conferma Password
          </label>
          <input
            id="confirmPassword"
            type="password"
            className="w-full px-4 py-3 bg-zinc-800/50 backdrop-blur-sm border border-zinc-700/50 rounded-xl focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500/50 transition-all duration-200 text-white placeholder:text-gray-500"
            placeholder="••••••••"
            value={confirmPassword}
            onChange={(e) => setConfirmPassword(e.target.value)}
            disabled={loading}
          />
        </div>

        {error && (
          <div className="p-4 bg-red-900/30 backdrop-blur-sm border border-red-800/50 rounded-xl">
            <p className="text-sm text-red-300">{error}</p>
          </div>
        )}

        {info && (
          <div className="p-4 bg-emerald-900/50 backdrop-blur-sm border border-emerald-700/50 rounded-xl">
            <p className="text-sm text-emerald-300">{info}</p>
          </div>
        )}

        {process.env.NODE_ENV === "development" && debugUser && (
          <div className="p-3 bg-yellow-900/30 backdrop-blur-sm border border-yellow-800/50 rounded-xl">
            <p className="text-xs font-mono text-yellow-300">
              <strong>DEV DEBUG:</strong> Email: {debugUser.email || "N/A"} | Verified: {debugUser.emailVerified ? "Yes" : "No"}
            </p>
          </div>
        )}

        <PrimaryButton
          type="submit"
          disabled={loading}
          className="w-full"
        >
          {loading ? (
            <span className="flex items-center justify-center space-x-2">
              <svg
                className="animate-spin h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
                  className="opacity-25"
                  cx="12"
                  cy="12"
                  r="10"
                  stroke="currentColor"
                  strokeWidth="4"
                />
                <path
                  className="opacity-75"
                  fill="currentColor"
                  d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                />
              </svg>
              <span>Creazione account...</span>
            </span>
          ) : (
            "Crea account"
          )}
        </PrimaryButton>
      </form>

      <div className="mt-6 pt-6 border-t border-zinc-800/50">
        <Link
          href="/"
          className="block w-full text-center text-sm text-gray-400 hover:text-gray-200 font-medium transition-colors duration-200"
        >
          Hai già un account? Accedi
        </Link>
      </div>
    </Card>
  );
}

